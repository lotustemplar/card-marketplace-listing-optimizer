...
SCRYFALL_REQUEST_INTERVAL_SECONDS = 0.25
...
class OptimizerSettings:
    ...
    def as_display_rows(self) -> list[dict[str, Any]]:
        ...
        rows.append({"Metric": "Mana Pool price source", "Value": "Live Mana Pool public card pages with throttled Scryfall exact-name metadata lookup, otherwise TCG fallback pricing"})
        return rows
...
def load_manapool_price_lookup(tcg_df: pd.DataFrame, column_map: dict[str, str]) -> tuple[dict[tuple[str, str, str], float], dict[tuple[str, str, str], str], str | None]:
    ...
    try:
        ...
        warning = None
        if misses:
            warning = f"Mana Pool public lookup matched {len(price_lookup)} exact card page(s) with throttled Scryfall metadata lookup. {misses} row key(s) fell back to TCG pricing."
