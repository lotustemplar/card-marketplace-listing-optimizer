...
USER_AGENT = "CardMarketplaceListingOptimizer/0.8 (+https://github.com/lotustemplar/card-marketplace-listing-optimizer)"
        ...
        rows.append(
            {
                ...
                "Value": "Mana Pool API /card_info for direct matches, plus /prices/variants for manual unresolved-match options",
            }
        )
        return rows
...
def fetch_manapool_variant_candidates(
    manapool_api_key: str | None,
    manapool_email: str | None,
) -> tuple[dict[str, list[dict[str, Any]]], str | None]:
    endpoint_url = f"{MANAPOOL_API_BASE_URL}{MANAPOOL_VARIANT_PRICES_ENDPOINT}"
    headers = build_manapool_headers(manapool_api_key, manapool_email)
    response_payload = request_json(endpoint_url, headers=headers)
    data = response_payload.get("data", []) if isinstance(response_payload, dict) else []
    grouped: dict[str, list[dict[str, Any]]] = {}
    for item in data:
        if not isinstance(item, dict):
            continue
        normalized_name = normalize_header(item.get("name", ""))
        if normalized_name:
            grouped.setdefault(normalized_name, []).append(item)
    return grouped, None


def build_candidate_option(card: dict[str, Any], source: str) -> dict[str, Any] | None:
    if source == "card_info":
        ...
        set_display = safe_text(card.get("set_name", "")) or safe_text(card.get("set_code", "")) or "Unknown set"
        card_number = safe_text(card.get("card_number", ""))
        label = f"{safe_text(card.get('name', 'Unknown card'))} | {set_display}"
        if card_number:
            label += f" | #{card_number}"
        label += f" | ${price:.2f}"
        return {
            ...
            "reason": f"Mana Pool manual override via card_info: {label}",
        }

    cents_value, parse_error = try_parse_number(card.get("low_price"))
    if parse_error or cents_value is None:
        return None
    price = round(cents_value / 100.0, 2)
    set_code = safe_text(card.get("set_code", ""))
    card_number = safe_text(card.get("number", ""))
    label = f"{safe_text(card.get('name', 'Unknown card'))} | {set_code or 'Unknown set'}"
    if card_number:
        label += f" | #{card_number}"
    label += f" | ${price:.2f}"
    if safe_text(card.get("url", "")):
        label += f" | {safe_text(card.get('url', ''))}"
    return {
        "label": label,
        "price": price,
        "name": safe_text(card.get("name", "")),
    ...
    for product_name, set_name, card_number in unique_keys:
        ...
        if row_key in overrides:
            ...
            if parse_error is None and override_price is not None:
                ...
                source_lookup[row_tuple] = safe_text(override.get("reason", "Mana Pool manual override"))
                manual_override_count += 1
                continue

        card_info_candidates = cards_by_name.get(normalize_header(product_name), [])
        match = choose_manapool_match(card_info_candidates, product_name, set_name, card_number)
        if match:
        ...
        for candidate in card_info_candidates:
            option = build_candidate_option(candidate, "card_info")
        ...
        if variant_candidates_by_name is None:
            try:
                variant_candidates_by_name, variant_warning = fetch_manapool_variant_candidates(manapool_api_key, manapool_email)
            except Exception as exc:
        ...
        for candidate in variant_candidates_by_name.get(normalize_header(product_name), []):
            option = build_candidate_option(candidate, "variant")
            if not option or option["label"] in seen_labels:
                continue
            seen_labels.add(option["label"])
            options.append(option)
