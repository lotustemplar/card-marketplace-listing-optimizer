...
def discover_manapool_cards_endpoint() -> tuple[str, str]:
    ...
    return best_method, parse.urljoin(MANAPOOL_BASE_URL, best_path)
...
def choose_manapool_match(cards: list[dict[str, Any]], product_name: str, set_name: str, card_number: str) -> dict[str, Any] | None:
    ...
    def score(card: dict[str, Any]) -> tuple[int, int, int]:
        ...
        return (set_match, number_match, name_match)
...
def load_manapool_price_lookup(
    ...
    for product_name, set_name, card_number in unique_keys:
        ...
        if not match:
            misses += 1
            continue
        from_price_cents = match.get("from_price_cents")
        cents_value, parse_error = try_parse_number(from_price_cents)
...
def build_analysis_dataframe(summary: dict[str, Any], settings: OptimizerSettings) -> pd.DataFrame:
    rows = [
        ...
        {"Metric": "Cards priced from Mana Pool API", "Value": summary["manapool_public_price_count"]},
