...
SCRYFALL_REQUEST_INTERVAL_SECONDS = 0.35
SCRYFALL_REQUEST_TIMEOUT_SECONDS = 10
MANAPOOL_PAGE_TIMEOUT_SECONDS = 10
MAX_MANAPOOL_LOOKUPS_PER_RUN = 60
MAX_MANAPOOL_LOOKUP_SECONDS = 25
...
class OptimizerSettings:
    ...
    def as_display_rows(self) -> list[dict[str, Any]]:
        ...
        rows.append({"Metric": "TCGPlayer Direct fee model", "Value": "< $2.50 = 50% of item value; >= $2.50 = $1.12 + 8.95% + 2.5%"})
        rows.append({"Metric": "Mana Pool price source", "Value": "Guarded Mana Pool public page lookup with capped Scryfall metadata requests, otherwise TCG fallback pricing"})
        return rows
...
def fetch_json(url: str, *, throttle_scryfall: bool = False) -> dict[str, Any]:
    ...
    try:
        with request.urlopen(req, timeout=SCRYFALL_REQUEST_TIMEOUT_SECONDS) as response:
...
def fetch_text(url: str) -> str:
    ...
    try:
        with request.urlopen(req, timeout=MANAPOOL_PAGE_TIMEOUT_SECONDS) as response:
...
def resolve_manapool_card_page(product_name: str, set_name: str, card_number: str, search_cache: dict[str, list[dict[str, Any]]]) -> str | None:
...
def load_manapool_price_lookup(tcg_df: pd.DataFrame, column_map: dict[str, str]) -> tuple[dict[tuple[str, str, str], float], dict[tuple[str, str, str], str], str | None]:
    ...
    page_cache: dict[str, float | None] = {}
    misses = 0
    skipped_due_to_guard = 0
    started_at = time.monotonic()

    try:
        for index, (product_name, set_name, card_number) in enumerate(unique_keys):
            if index >= MAX_MANAPOOL_LOOKUPS_PER_RUN or (time.monotonic() - started_at) > MAX_MANAPOOL_LOOKUP_SECONDS:
                skipped_due_to_guard += 1
                continue
            ...
            if page_url not in page_cache:
                page_cache[page_url] = extract_lowest_public_price(fetch_text(page_url))
            price = page_cache[page_url]
            if price is None:
                misses += 1
                continue
        ...
        warnings = []
        if misses:
            warnings.append(f"Mana Pool public lookup matched {len(price_lookup)} exact card page(s). {misses} row key(s) fell back to TCG pricing.")
        if skipped_due_to_guard:
            warnings.append(f"Guardrails limited live Mana Pool lookups for {skipped_due_to_guard} additional row key(s) on this run, so those rows used TCG fallback pricing to keep the app responsive.")
...
def build_analysis_dataframe(summary: dict[str, Any], settings: OptimizerSettings) -> pd.DataFrame:
    rows = [
        ...
        {"Metric": "Tracked shipping review warning", "Value": f"Review Manapool cards at or above ${settings.tracked_shipping_threshold:.2f}; tracked shipping can add about ${settings.tracked_shipping_cost:.2f} to fulfillment."},
        {"Metric": "", "Value": ""},
...
def process_files(tcgplayer_bytes: bytes, settings: OptimizerSettings, manapool_api_key: str | None = None, manapool_email: str | None = None) -> ProcessResult:
    ...
    if missing_columns:
        errors_df = pd.DataFrame([build_error_row(None, f"Required CSV column missing: {', '.join(missing_columns)}", source_columns if source_columns else ["Original row data"])])
        summary = {"total_rows_imported": int(len(tcg_df)), "total_quantity_imported": 0, "total_cards_assigned_manapool": 0, "total_cards_assigned_direct": 0, "total_estimated_manapool_net": 0.0, "total_estimated_direct_net": 0.0, "combined_estimated_net": 0.0, "average_direct_bump_pct": 0.0, "skipped_error_rows": int(len(tcg_df)), "missing_price_data_count": 0, "forced_manapool_min_count": 0, "direct_bump_exceeded_count": 0, "manapool_public_price_count": 0, "manapool_fallback_price_count": 0, "tracked_shipping_review_count": 0}
        analysis_df = build_analysis_dataframe(summary, settings)
        return ProcessResult(pd.DataFrame(columns=DISPLAY_COLUMNS_MANAPOOL + ["_forced_min", "_bump_exceeded"]), pd.DataFrame(columns=DISPLAY_COLUMNS_DIRECT + ["_bump_exceeded"]), pd.DataFrame(columns=DISPLAY_COLUMNS_MANAPOOL), pd.DataFrame(columns=DISPLAY_COLUMNS_DIRECT), pd.DataFrame(columns=source_columns), pd.DataFrame(columns=source_columns), errors_df, analysis_df, summary, settings, missing_columns, "Required columns are missing from the TCGPlayer export.")
    ...
    for _, row in tcg_df.iterrows():
        row_errors: list[str] = []
        numeric_fields = {"market_price": "TCG Market Price", "direct_low": "TCG Direct Low", "low_price": "TCG Low Price", "total_quantity": "Total Quantity"}
        ...
        row_base = {"TCGplayer Id": safe_text(row.get(column_map["TCGplayer Id"], "")), "Product Line": safe_text(row.get(column_map["Product Line"], "")), "Set Name": set_name, "Product Name": product_name, "Number": card_number, "Rarity": safe_text(row.get(column_map.get("Rarity", ""), "")) if column_map.get("Rarity") else "", "Condition": safe_text(row.get(column_map.get("Condition", ""), "")) if column_map.get("Condition") else "", "Quantity": normalize_quantity(quantity)}
        if destination == "direct":
            direct_csv_rows.append(build_upload_row(row, source_columns, column_map, quantity, direct_listing_price))
            direct_rows.append({**row_base, "Direct Listing Price": round(direct_listing_price, 2) if direct_listing_price is not None else None, "Direct Net": round(direct_net, 2) if direct_net is not None else None, "Manapool Price": round(manapool_price, 2), "Manapool Net": round(manapool_net, 2), "Direct Bump %": direct_bump_pct, "Reason": "; ".join(reason_parts), "_bump_exceeded": bump_exceeded})
        else:
            ...
            manapool_rows.append({**row_base, "Manapool Price": round(manapool_price, 2), "Manapool Net": round(manapool_net, 2), "Base Direct Price": round(base_direct_price, 2) if base_direct_price is not None else None, "Base Direct Net": round(base_direct_net, 2) if base_direct_net is not None else None, "Required Direct Price": round(direct_listing_price, 2) if direct_listing_price is not None else None, "Direct Bump %": direct_bump_pct, "Reason": "; ".join(reason_parts), "_forced_min": forced_min, "_bump_exceeded": bump_exceeded})
    ...
    summary = {"total_rows_imported": int(len(tcg_df)), "total_quantity_imported": normalize_quantity(total_quantity_imported) if total_quantity_imported else 0, "total_cards_assigned_manapool": int(pd.to_numeric(manapool_full_df["Quantity"], errors="coerce").sum()) if not manapool_full_df.empty else 0, "total_cards_assigned_direct": int(pd.to_numeric(direct_full_df["Quantity"], errors="coerce").sum()) if not direct_full_df.empty else 0, "total_estimated_manapool_net": round(manapool_total_net, 2), "total_estimated_direct_net": round(direct_total_net, 2), "combined_estimated_net": round(manapool_total_net + direct_total_net, 2), "average_direct_bump_pct": direct_bump_average, "skipped_error_rows": int(len(errors_df)), "missing_price_data_count": missing_price_data_count, "forced_manapool_min_count": forced_manapool_min_count, "direct_bump_exceeded_count": direct_bump_exceeded_count, "manapool_public_price_count": manapool_public_price_count, "manapool_fallback_price_count": manapool_fallback_price_count, "tracked_shipping_review_count": tracked_shipping_review_count}
    analysis_df = build_analysis_dataframe(summary, settings)
    warnings: list[str] = []
    ...
    return ProcessResult(manapool_full_df.sort_values("Product Name", key=lambda series: series.astype(str).str.lower()).reset_index(drop=True), direct_full_df.sort_values("Product Name", key=lambda series: series.astype(str).str.lower()).reset_index(drop=True), manapool_preview_df, direct_preview_df, manapool_csv_df, direct_csv_df, errors_df, analysis_df, summary, settings, missing_columns, "\n\n".join(warnings) if warnings else None)
