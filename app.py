...
def main() -> None:
    ...
    st.success("Mana Pool pricing now assumes Near Mint nonfoil by default, uses Near Mint Foil pricing only when the TCGPlayer condition says foil, treats The List Reprints as PLST during set matching, includes hypothetical all-Mana-Pool vs all-Direct net totals for comparison, keeps Rarity = T rows permanently on TCGPlayer Direct, and still enforces the Direct bump cap on non-token rows.")
