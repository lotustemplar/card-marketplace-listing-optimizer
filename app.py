...
def run_low_price_inspection(file_bytes: bytes, threshold: float) -> None:
    ...
    for index, (_, row) in enumerate(dataframe.iterrows(), start=1):
        ...
        reversed_sequence = total_rows - index + 1
...
def render_low_price_inspection_page() -> None:
    ...
    st.info("This tool uses the ManaBox 'Purchase price' column and reports each low-priced card in reverse scan order, so the last scanned card becomes Sequence 1. 'CSV Row' still shows the original row in the uploaded file, including the header.")
    ...
    if low_rows_df.empty:
        ...
        st.warning(f"Found {len(low_rows_df)} row(s) below ${threshold:.2f}. These are shown in reverse scan order, where the last scanned card is Sequence 1.")
